...
export default {
  ...
  methods: {
    ...
    initPicScroll () {
      const picWrapper = this.$refs.picWrapper
      const picList = this.$refs.picList
      let width = 120
      let margin = 10
      let length = this.seller.pics.length
      if (length <= 3) return
      picList.style.width = length * (width + margin) - margin + 'px'
      this.picWrapper = new BScroll(picWrapper, {
        scrollX: true,
